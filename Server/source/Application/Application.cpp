...
void Application::run()
{
    ...
    while(true)
    {
        ...
        // if(sound.is_playing())
        // {
        //     std::cout << "received message, but previous one is still playing" << std::endl;
        //     continue;
        // }

        raw_data.size = client_message.message.size();
        raw_data.data = new char[raw_data.size + 1];
        memcpy(raw_data.data, client_message.message.c_str(), client_message.received);
